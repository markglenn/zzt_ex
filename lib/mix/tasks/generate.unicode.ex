defmodule Mix.Tasks.Generate.Unicode do
  @lookup """
    #
  ...
  def run(_args) do
    {:ok, file} = File.open("lib/zzt_ex/glyph.ex", [:write])

    IO.binwrite(file, "defmodule ZZTEx.Glyph do\n")
    IO.binwrite(file, "  @doc \"Convert a CP437 glyph byte to a unicode character\"\n")
    IO.binwrite(file, "  @spec to_unicode(byte) :: char\n")
    ...
    IO.binwrite(file, "  def to_unicode(c), do: c\n")
  ...
  defp to_func(line) do
    ["0x" <> ascii, "0x" <> unicode] = String.split(line, "\t")

    if String.to_integer(ascii, 16) == String.to_integer(unicode, 16) do
      ""
    else
      "def to_unicode(0x#{String.upcase(ascii)}), do: 0x#{String.upcase(unicode)}"
    end
  end
end
